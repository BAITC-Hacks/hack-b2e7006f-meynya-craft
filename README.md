...
| Адрес | Назначение |
| --- | --- |
| `/` | Вход для бизнеса и студентов |
| `/create` | Описание, уточнения, редактирование, подтверждение публикации |
| `/catalog` | Каталог, сортировка по score и фильтры темы/уровня |
| `/tasks/[id]` | Полная карточка, разбивка score, отправка отклика |
| `/business` | Все задачи демо и количество ожидающих откликов |
| `/business/[id]` | Редактирование с пересчётом score, публикация черновика, Accept/Reject |

Переключатель Business / Student меняет экран, а не права доступа: авторизации нет.
Используйте ссылки приложения в одной вкладке. После подтверждения на экране создания
есть переходы в каталог и к управлению задачей. Ручные правки в Business сохраняются
сразу. Принятая задача становится «В работе» и закрывается для новых откликов.
